defmodule Castle do
  ...
  after any function step of the project's own, so that the relup describes the
  tree that gets packed rather than the tree partway through being built. With
  no `:tar` there is nothing to precede and the step is appended. It does
  nothing unless the release names a baseline; see *Upgrades* below.
  ...
  **`customize/1` does not read this option and does not check it.** It is an
  ordinary release option; Mix keeps the options it does not recognise, so it
